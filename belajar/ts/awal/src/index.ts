//variabel
const nama: string = "kamal"
console.info(`variabel@ ${nama}`)
console.info(" ")

// Array
const iniArray: (string | number)[] = [
    "apel",
    "38",
    "zombie",
    "5000"
...
for (const element in iniObjeck) {
  const key = element as keyof typeof iniObjeck;
...
const tambahAwal:number = tambah(4,7)
console.info(tambahAwal)
console.info(" ")

//if else
function ifElse(angka:number) {
    if (angka < 0) {
...
const ifElseHasil: string= ifElse(-7)
console.info(ifElseHasil)
console.info(" ")

//switch
const iniSwitch: number = 80
switch (true) {
    case iniSwitch == 100:
