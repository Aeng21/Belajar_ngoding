const API_URL = 'http://localhost:3000/api/mob';
...
const tipeTambah = document.getElementById('tipeTambah') as HTMLInputElement;
const healthTambah = document.getElementById('healthTambah') as HTMLInputElement;
const tbody = document.getElementById('tbodyMob') as HTMLTableSectionElement;
...
const editTipe = document.getElementById('editTipe') as HTMLInputElement;
...
function openEditModal(id: string, nama: string, tipe: string, health: string) {
    currentEditId = id;
    editNama.value = nama;
    editTipe.value = tipe;
...
function renderData(data: any[]) {
    ...
    data.forEach(item => {
        html += `
          <tr>
            ...
            <td>${item.health}</td>
            <td>
              <button class="action-btn edit-btn" data-id="${item.id}" data-nama="${item.nama}" data-tipe="${item.tipe}" data-health="${item.health}">Edit</button>
    ...
    document.querySelectorAll('.edit-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            openEditModal(target.dataset.id!, target.dataset.nama!, target.dataset.tipe!, target.dataset.health!);
        });
    });
...
async function createData(nama: string, tipe: string, health: string) {
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ nama, tipe, health: parseInt(health) })
        });
        const result = await res.json();
        if (result.success) {
...
async function updateData(id: string, nama: string, tipe: string, health: string) {
    try {
...
formTambah.addEventListener('submit', (e) => {
    ...
    const tipe = tipeTambah.value.trim();
    const health = healthTambah.value.trim();
    if (!nama || !tipe || !health) {
        alert('Semua field harus diisi!');
        return;
    }
    createData(nama, tipe, health);
});

formEdit.addEventListener('submit', (e) => {
    e.preventDefault();
    const nama = editNama.value.trim();
    const tipe = editTipe.value.trim();
    ...
    updateData(currentEditId, nama, tipe, health);
});
