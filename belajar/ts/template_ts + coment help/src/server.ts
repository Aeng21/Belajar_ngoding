...
// mengimport export default untuk masing-masing router (player, mob, enemy). dari folder routes
// File-file ini berisi rute-rute spesifik (misal GET /api/player, POST /api/player, dll.).
...
// Tanpa ini, req.body akan undefined untuk request JSON.
...
// (req, res) => { ... } : fungsi yang dijalankan ketika ada request GET ke '/'.
// Menangani request GET ke '/' (root). Mengembalikan pesan sukses dan daftar endpoint yang tersedia.
app.get('/', (req: Request, res: Response) => {
  // res.json() : mengirim response dalam bentuk JSON ke client.
  res.json({
...
// err: error yang terjadi, req: request dari client, res: response untuk client, next: meneruskan ke middleware berikutnya
app.use((err: any, req: Request, res: Response, next: NextFunction) => {
  console.error(err.stack); // menampilkan tumpukan error di console
  res.status(500).json({
    success: false,
    message: 'Terjadi kesalahan pada server',
    error: err.message, // pesan error dikirim ke client (untuk debugging, bisa dihapus di production)
  });
});
